...
void GpioMcuSetInterrupt( Gpio_t *obj, IrqModes irqMode, IrqPriorities irqPriority, GpioIrqHandler *irqHandler )
{
    if( obj->pin < IOE_0 )
    {
        ...
        case IRQ_LOW_PRIORITY:
            priority = (configLIBRARY_MAX_SYSCALL_INTERRUPT_PRIORITY < 3) ? 3:configLIBRARY_MAX_SYSCALL_INTERRUPT_PRIORITY;
            break;
        case IRQ_MEDIUM_PRIORITY:
            priority = (configLIBRARY_MAX_SYSCALL_INTERRUPT_PRIORITY < 2) ? 2:configLIBRARY_MAX_SYSCALL_INTERRUPT_PRIORITY;
            break;
        case IRQ_HIGH_PRIORITY:
            priority = (configLIBRARY_MAX_SYSCALL_INTERRUPT_PRIORITY < 1) ? 1:configLIBRARY_MAX_SYSCALL_INTERRUPT_PRIORITY;
            break;
        case IRQ_VERY_HIGH_PRIORITY:
        default:
            priority = (configLIBRARY_MAX_SYSCALL_INTERRUPT_PRIORITY < 0) ? 0:configLIBRARY_MAX_SYSCALL_INTERRUPT_PRIORITY;
            break;
        }
...
void HAL_GPIO_EXTI_Callback( uint16_t gpioPin )
{
    ...
    /* Call the interrupt service routine for other pin */
//    TODO implement extcallback for all other pin that rise an interrupt
//    GPIO_EXTI_Callback( gpioPin );
