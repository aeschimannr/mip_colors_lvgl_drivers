...
void sharp_mip_init(void) {
  /* These displays have nothing to initialize */
	...
	HAL_GPIO_WritePin(LCD_nSS_SPI3_GPIO_Port, LCD_nSS_SPI3_Pin, 1);
	HAL_SPI_Transmit(&hspi3, &cmd, sizeof( cmd ), 1000);
	HAL_GPIO_WritePin(LCD_nSS_SPI3_GPIO_Port, LCD_nSS_SPI3_Pin, 0);

	/* enable disp */
	HAL_GPIO_WritePin(LCD_DISP_ON_nOFF_GPIO_Port, LCD_DISP_ON_nOFF_Pin, 1);
}
